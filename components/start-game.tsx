...
export function StartGameComponent() {
  ...
  const handleStartGame = (code: string) => {
    if (code && screenName) {
      router.push(`/game/${code}?player=${encodeURIComponent(screenName)}&creator=true`)
    }
  }

  const handleJoinGame = (code: string) => {
    if (code && screenName) {
      router.push(`/game/${code}?player=${encodeURIComponent(screenName)}&creator=false`)
    }
  }
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <Card className="w-[350px]">
        <CardHeader>
          ...
          <CardDescription>Enter your screen name and generate a new game or join an existing one</CardDescription>
        </CardHeader>
        <CardContent>
